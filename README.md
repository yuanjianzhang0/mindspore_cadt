...
<p align="center">
    ...
    <a href="https://e.huawei.com/en/products/computing/ascend"><img alt="Ascend 310B" src="https://img.shields.io/badge/Hardware-Ascend_310B-red"></a>
    <a href="https://github.com/QwenLM/Qwen2.5"><img alt="Qwen LLM" src="https://img.shields.io/badge/Cognitive_Engine-Qwen_0.5B-purple"></a>
...
```shell
git clone [https://github.com/your-repo/Drowning-Detection-System.git](https://github.com/your-repo/Drowning-Detection-System.git)
...
1. **解决传统单模态视觉盲区**：
* 针对野外水域低能见度、光照变化等恶劣条件，引入 UUV 声呐与医疗贴片，实现视觉失效下的跨介质数据互补。


2. **边缘侧极致性能优化**：
* 模型不依赖云端算力。核心感知网络 CADT 通过 `mindspore_lite` 转化为 FP16 精度静态图，在昇腾 310B 上实现单帧数十毫秒级的超低延迟。


3. **LLM 赋能的认知决策 (Cognitive Engine)**：
* 抛弃传统的 `if-else` 硬编码规则。由 Qwen 大模型消化复杂的环境上下文（如高海浪、心率异常），输出确定性的 JSON 救援策略。


4. **即插即用的多智能体调度**：
* 无缝集成 DR-MAPPO RL 调度引擎，驱动 USV (无人水面艇) 投放 AED 及 UUV 协同探测，真正实现从感知到执行的自主闭环。
...
```bibtex
@software{cadt_drowning_detection_2026,
    title = {Cross-modal Aquatic Distress Transformer (CADT) for Autonomous Multi-Agent Rescue},
    author = {Your Team Name},
    year = {2026},
    url = {[https://github.com/your-repo/Drowning-Detection-System](https://github.com/your-repo/Drowning-Detection-System)},
    note = {Powered by MindSpore and Ascend NPU}
}
